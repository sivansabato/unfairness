%by default, this script generates decision tree classifiers.
%set 'classifier_type' to 'linear' to generate linear classifiers.
...
alltemp = calculate_classifier(X,catColTemp, Y, trainsize, data_screened(:,powstate),classifier_type);
...
filemat(:,5) = I';
filemat(:,6) = J';
filename = sprintf('census_commands_%s_%s.csv', classifier_type, datetime('now','Format','yyyy-MM-dd__HH_mm_ss'));
